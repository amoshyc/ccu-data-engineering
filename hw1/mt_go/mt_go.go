...
func mt_ngrams(text_path, term_path, output_path string) []Item {
	terms := read_file(term_path)

	channel := make(chan map[string]*Item)
	f, _ := os.Open(text_path)
	scanner := bufio.NewScanner(f)
	chunk := make([]string, 0)
	chunk_cnt := 0
	chunk_size := 1000000
	for {
		for len(chunk) < chunk_size && scanner.Scan() {
			chunk = append(chunk, scanner.Text())
		}
		go func(data []string, line_id int) {
			channel <- ngrams(line_id, data, &terms)
		}(chunk, chunk_cnt * chunk_size)
		chunk = make([]string, 0)
		chunk_cnt += 1
		if !scanner.Scan() {
			break
...
func main() {
	...
	text_path := os.Args[2]
	out_path := os.Args[3]
	mt_ngrams(text_path, term_path, out_path)
}
